...
class ReportController extends Controller
{
    ...
    public function index(Request $request): Response
    {
        ...
        $daysElapsed = $isCurrentMonth ? now()->day : Carbon::createFromFormat('Y-m', $period)->daysInMonth;
        ...
        $insight = [
            ...
            'most_wasteful_day' => $mostWastefulDay ? [
                'date' => Carbon::parse($mostWastefulDay['date'])->translatedFormat('d M Y'),
    ...
    {
        $topCategories = collect($data['byCategory'])->take(5)->map(fn ($c) => "  {$c['emoji']} {$c['name']}: Rp ".number_format($c['total'], 0, ',', '.')
        )->join("\n");
