...
class User extends Authenticatable
{
    ...
    public function wallets(): HasMany      { return $this->hasMany(UserWallet::class)->orderBy('sort_order'); }
    public function transactions(): HasMany { return $this->hasMany(Transaction::class); }
    public function savingGoals(): HasMany  { return $this->hasMany(SavingGoal::class); }
    ...
    public function assets(): HasMany        { return $this->hasMany(UserAsset::class)->where('is_active', true)->orderBy('sort_order'); }
    public function receiptScans(): HasMany  { return $this->hasMany(ReceiptScan::class); }
    ...
    public function hasActiveSubscription(): bool
    {
        ...
        if (!$sub) return false;
        if ($sub->plan === 'trial' && $sub->trial_ends_at?->isPast()) return false;
        return $sub->status === 'active';
    }
    public function totalBalance(): float   { return (float) $this->wallets()->where('is_active', true)->sum('balance'); }
    public function activeWallets()         { return $this->wallets()->where('is_active', true)->get(); }
}
