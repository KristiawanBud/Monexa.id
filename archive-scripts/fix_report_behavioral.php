...
patchFile($file, [
    ...
    '<div class="flex items-end gap-2.5 overflow-x-auto" style="height:130px;">' => '<div class="flex items-end justify-between gap-1.5" style="height:170px;">',
    ...
    '<div
            ...
            >
              <div class="flex items-end gap-1" style="height:100px;">' => '<div
              v-for="item in barData"
              :key="item.period"
    ...
    'class="text-[21px] font-bold text-slate-800 text-left leading-tight tracking-tight mt-1 truncate">
            {{ formatShort(totalIncome) }}' => 'class="font-bold text-slate-800 text-left leading-tight tracking-tight mt-1 truncate"
            style="font-size:clamp(20px,6vw,28px);"
          >
            {{ formatShort(totalIncome) }}',

    'class="text-[21px] font-bold text-slate-800 text-left leading-tight tracking-tight mt-1 truncate">
            {{ formatShort(totalExpense) }}' => 'class="font-bold text-slate-800 text-left leading-tight tracking-tight mt-1 truncate"
            style="font-size:clamp(20px,6vw,28px);"
          >
            {{ formatShort(totalExpense) }}',

    // FIX 5: Donut diameter pakai clamp
    'style="width:128px;height:128px;"' => 'style="width:clamp(120px,35vw,140px);height:clamp(120px,35vw,140px);"',
    ...
    'class="flex items-center gap-1.5 bg-white text-[#2F6BFF] font-semibold text-[12.5px]
                     ...
                     active:scale-95 px-3.5 py-2.5"' => 'class="flex items-center gap-1.5 bg-white text-[#2F6BFF] font-semibold text-[12.5px]
                     rounded-full shadow-[0_4px_14px_rgba(0,0,0,0.12)] transition-transform
                     active:scale-95 px-3.5"
              style="min-height:44px;"',

    'class="relative w-10 h-10 rounded-full bg-white/15 backdrop-blur-sm flex items-center
                     justify-center text-white transition-transform active:scale-95"' => 'class="relative rounded-full bg-white/15 backdrop-blur-sm flex items-center
