...
patchFile($file, [
    ...
    '<div class="px-5 relative z-20 grid grid-cols-2 gap-3" style="margin-top:-36px;">' => '<div class="px-5 relative z-20 grid grid-cols-1 gap-3" style="margin-top:-36px;">',

    'style="font-size:clamp(14px,4.4vw,19px); line-height:1.25;"
          >
            {{ formatRupiah(totalIncome) }}' => 'style="font-size:clamp(20px,6.5vw,26px); line-height:1.2;"
          >
            {{ formatRupiah(totalIncome) }}',

    'style="font-size:clamp(14px,4.4vw,19px); line-height:1.25;"
          >
            {{ formatRupiah(totalExpense) }}' => 'style="font-size:clamp(20px,6.5vw,26px); line-height:1.2;"
          >
    ...
    'class="bg-gradient-to-br from-slate-900 to-slate-800 text-white rounded-[24px]
          ...
          >
            ...
            </div>' => 'class="bg-gradient-to-br from-slate-900 to-slate-800 text-white rounded-[24px]
    ...
    'style="font-size:clamp(18px,5.5vw,22px); line-height:1.1;">
              {{ healthScore }}' => 'style="font-size:24px; line-height:1.1;">
              {{ healthScore }}',

    'style="font-size:clamp(18px,5.5vw,22px); line-height:1.1;">
                  {{ runwayMonths }}' => 'style="font-size:clamp(26px,7vw,32px); font-weight:700; line-height:1.1;">
                  {{ runwayMonths }}',

    '<span class="text-white/60" style="font-size:10.5px;"> Bulan</span>' => '<span class="text-white/60" style="font-size:12px;"> Bulan dari target 6 bulan</span>',

    '<span class="text-right font-semibold flex-shrink-0 w-7">{{ savingRatio }}%</span>' => '<span class="text-right font-semibold flex-shrink-0 w-9">{{ savingRatio }}%</span>',
    '<span class="text-right font-semibold flex-shrink-0 w-7">{{ budgetDiscipline }}%</span>' => '<span class="text-right font-semibold flex-shrink-0 w-9">{{ budgetDiscipline }}%</span>',
    '<span class="text-right font-semibold flex-shrink-0 w-7">{{ runwayScore }}%</span>' => '<span class="text-right font-semibold flex-shrink-0 w-9">{{ runwayScore }}%</span>',
]);
