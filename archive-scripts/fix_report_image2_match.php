...
<template>
  <AppLayout>
    <div class="report-tw bg-slate-50 min-h-screen pb-8">
      ...
      >
        ...
        <div class="relative z-10 flex justify-between items-start mb-3">
          <div class="min-w-0 flex-1 pr-2">
            <h1 class="font-bold text-white" style="font-size:20px; line-height:1.2;">
              📊 Laporan
            </h1>
            <p class="text-white/75 mt-1" style="font-size:12px; line-height:1.4;">
              Pantau keuanganmu, budget health, dan dana darurat kamu.
            </p>
          </div>
        ...
        <div class="relative inline-block z-10 mt-2">
          <button
            type="button"
      ...
      <!-- ═══ SUMMARY — angka penuh, font scaled biar muat ═══ -->
      <div class="px-5 relative z-20 grid grid-cols-2 gap-3" style="margin-top:-36px;">
        <div class="bg-white rounded-[20px] shadow-[0_8px_24px_rgba(15,23,42,0.10)] p-4 min-w-0">
          <div class="flex items-center gap-1.5 mb-1">
            ...
            <span class="text-slate-400 truncate" style="font-size:11px;">Total Pemasukan</span>
          </div>
          <div
          ...
          <div
            :class="[
              'font-medium text-left mt-1 truncate',
        ...
        <div class="bg-white rounded-[20px] shadow-[0_8px_24px_rgba(15,23,42,0.10)] p-4 min-w-0">
          <div class="flex items-center gap-1.5 mb-1">
            <div class="w-6 h-6 rounded-full bg-rose-50 flex items-center justify-center flex-shrink-0">
              <TrendingDown :size="12" class="text-rose-500" />
            </div>
            <span class="text-slate-400 truncate" style="font-size:11px;">Total Pengeluaran</span>
          </div>
          <div
            class="font-bold text-slate-800 text-left tracking-tight overflow-hidden text-ellipsis whitespace-nowrap"
            style="font-size:clamp(14px,4.4vw,19px); line-height:1.25;"
      ...
      <div class="px-5 pt-6 space-y-5">
        ...
        <!-- ═══ FILTER — section TERPISAH di atas Chart ═══ -->
        ...
        <!-- ═══ HEALTH + EMERGENCY — sejajar, teks di-scale ═══ -->
        <div class="grid grid-cols-2 gap-3">
          <div class="bg-white rounded-[24px] shadow-[0_8px_24px_rgba(15,23,42,0.06)] p-4 min-w-0">
            ...
            <div class="text-left">
              <span class="font-bold text-[#2F6BFF]" style="font-size:clamp(18px,5.5vw,22px); line-height:1.1;">
                {{ healthScore }}
              </span>
              <span class="text-slate-400" style="font-size:11px;">/100</span>
            </div>
            <div
              :class="['font-medium mb-3 mt-1 flex items-center gap-1 text-left', healthColorText]"
              style="font-size:11px;"
            >
              <AppIcon :slug="`health_tier_${healthStatus}`" class="w-3 h-3 inline-block flex-shrink-0">
                {{ defaultTierEmoji(healthStatus) }}
              </AppIcon>
              {{ healthLabel(healthStatus) }}
            </div>
            <div class="space-y-2">
              <div class="flex items-center gap-1.5" style="font-size:9.5px;">
          ...
          >
            ...
            <template v-if="runwayMonths !== null">
              <div class="text-left relative z-10 mb-2.5">
                <span class="font-bold" style="font-size:clamp(18px,5.5vw,22px); line-height:1.1;">
                  {{ runwayMonths }}
                </span>
                <span class="text-white/60" style="font-size:10.5px;"> Bulan</span>
              </div>
              <div class="h-1.5 bg-white/15 rounded-full overflow-hidden mb-2 relative z-10">
                <div
                  class="h-full bg-amber-400 rounded-full"
                  :style="`width:${Math.min((runwayMonths/6)*100,100)}%`"
                ></div>
              </div>
              <div class="text-white/45 relative z-10 text-left leading-snug" style="font-size:9.5px;">
                Target ideal: 6 bulan
              </div>
            </template>
            <div v-else class="text-white/60 relative z-10 text-left" style="font-size:11px;">
              Belum cukup data.
            </div>
            <PiggyBank :size="56" class="absolute -bottom-2 -right-2 text-white/[0.08]" />
          </div>
        </div>

        <!-- ═══ DONUT — angka penuh + Lihat Semua ═══ -->
        <div
          v-if="donutData.length > 0"
          class="bg-white rounded-[24px] shadow-[0_8px_24px_rgba(15,23,42,0.06)] p-5"
        >
          <div class="font-semibold text-slate-800 mb-4 text-left" style="font-size:16px;">
          ...
          <div class="flex flex-col items-center gap-5">
            ...
            <button
              type="button"
              class="w-full text-center text-[#2F6BFF] font-semibold flex items-center justify-center gap-1 pt-2"
              style="font-size:13px;"
            >
    ...
    <Teleport to="body">
      ...
      >
        <div class="bg-white rounded-t-[24px] w-full max-w-md p-6 pb-10">
          ...
          <div class="font-semibold mb-5 text-left" style="font-size:17px;">
...
function barHeight(v) {
  return Math.max(3, (v / maxBarVal.value) * 120)
}

function yAxisLabel(step) {
  const val = (maxBarVal.value * step) / 4
