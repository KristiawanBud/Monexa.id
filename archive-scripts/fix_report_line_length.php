    ...
    <div class="report-tw bg-slate-50 min-h-screen pb-8">
      ...
      <div
        class="relative overflow-visible bg-gradient-to-br from-[#2F6BFF] via-[#4F7FFF] to-[#1E4FD6]
               px-5 pt-6 pb-14 rounded-b-[24px] shadow-[0_12px_40px_rgba(47,107,255,0.20)]"
      >
        ...
        <div class="relative z-10 flex justify-between items-center mb-4">
          <h1 class="text-[22px] font-semibold text-white leading-none flex-shrink-0">
            📊 Laporan
          </h1>
          <div class="flex items-center gap-2 flex-shrink-0">
            ...
            >
              ...
              <span
                ...
                class="absolute -top-1 -right-1 bg-rose-500 text-white text-[9px] font-bold w-4 h-4
        ...
        <div class="relative inline-block z-10">
          <button
            ...
            class="flex items-center gap-2 bg-white/15 backdrop-blur-sm text-white text-[12.5px]
                   font-semibold rounded-full transition-transform active:scale-95 px-3.5 py-2.5"
          >
            <Calendar :size="14" /> {{ periodLabel }}
            <ChevronDown :size="14" :class="['transition-transform', showMonthMenu && 'rotate-180']" />
          </button>
          <div
            v-if="showMonthMenu"
          ...
          >
            <button
              ...
              :class="[
                'block w-full text-left px-4 py-2.5 text-[13px]',
      ...
      <div class="px-5 relative z-20 grid grid-cols-2 gap-3" style="margin-top:-36px;">
        <div class="bg-white rounded-[20px] shadow-[0_8px_24px_rgba(15,23,42,0.10)] p-4 min-w-0">
          <div class="text-[11px] font-normal text-slate-400 text-left whitespace-nowrap">
            ↑ Pemasukan
          </div>
          <div class="text-[21px] font-bold text-slate-800 text-left leading-tight tracking-tight mt-1 truncate">
            {{ formatShort(totalIncome) }}
          </div>
          <div
            :class="[
              'text-[11px] font-medium text-left mt-1.5 truncate',
      ...
      <div class="px-5 pt-6 space-y-5">
        ...
        <div class="bg-white rounded-[24px] shadow-[0_8px_24px_rgba(15,23,42,0.06)] p-5">
          ...
          <div class="flex items-end gap-2.5 overflow-x-auto" style="height:130px;">
            ...
            >
              ...
              <div
                :class="['text-[10px] mt-1.5 font-normal', item.active ? 'text-[#2F6BFF] font-semibold' : 'text-slate-400']"
              >
                {{ item.period }}
              </div>
            </div>
          ...
          <div class="flex gap-4 justify-center pt-3 mt-1 border-t border-slate-100">
            <div class="flex items-center gap-1.5 text-[11px] text-slate-500 font-normal">
        ...
        <!-- ═══ HEALTH + EMERGENCY (stack vertikal) ═══ -->
        <div class="space-y-4">
          <div class="bg-white rounded-[24px] shadow-[0_8px_24px_rgba(15,23,42,0.06)] p-5">
            ...
            <div
              :class="[
                'text-[12px] font-medium mb-4 mt-1 flex items-center gap-1.5 text-left',
                healthColorText
              ]"
            >
            ...
            <template v-if="runwayMonths !== null">
              ...
              <div class="h-2 bg-white/15 rounded-full overflow-hidden mb-2 relative z-10">
                <div
                  ...
                  :style="`width:${Math.min((runwayMonths/6)*100,100)}%`"
                ></div>
              </div>
              <div class="text-[11px] text-white/45 relative z-10 text-left">
                Target ideal: 6 bulan pengeluaran
              </div>
            </template>
            <div v-else class="text-[12px] text-white/60 relative z-10 text-left">
              Belum cukup data.
            </div>
          ...
          <div class="flex flex-col items-center gap-5">
            ...
            <div class="w-full flex flex-col gap-3">
              ...
              >
                ...
                ></span>
                <span class="flex-1 text-slate-700 font-medium text-left min-w-0 truncate">
                  {{ cat.emoji }} {{ cat.category }}
                </span>
                <span class="text-slate-400 text-[12px] flex-shrink-0">{{ formatShort(cat.total) }}</span>
                <span class="font-semibold text-slate-800 text-[12px] w-10 text-right flex-shrink-0">
                  {{ cat.percent }}%
                </span>
              </div>
            </div>
        ...
        >
          ...
          <div v-for="b in budgets" :key="b.category" class="mb-4 last:mb-0">
            <div class="flex justify-between text-[13px] mb-2 gap-2">
              ...
              <span class="text-slate-400 text-[11px] flex-shrink-0">
            ...
            <div class="h-1.5 bg-slate-100 rounded-full overflow-hidden">
              <div
                :class="[
                  'h-full rounded-full',
                  b.status === 'over' ? 'bg-rose-500' : b.status === 'warn' ? 'bg-amber-500' : 'bg-emerald-500'
                ]"
        ...
        <div>
          ...
          <div class="grid grid-cols-2 gap-3">
            <div class="bg-white rounded-[18px] shadow-[0_8px_24px_rgba(15,23,42,0.06)] p-3.5">
              ...
              <div class="text-[13.5px] font-semibold leading-snug mt-0.5 break-words">
                {{ insight.biggest_income ? formatShortVal(insight.biggest_income.amount) : dash }}
              </div>
            </div>
            <div class="bg-white rounded-[18px] shadow-[0_8px_24px_rgba(15,23,42,0.06)] p-3.5">
              ...
              <div class="text-[13.5px] font-semibold leading-snug mt-0.5 break-words">
                {{ insight.biggest_expense ? formatShortVal(insight.biggest_expense.amount) : dash }}
              </div>
            </div>
            <div class="bg-white rounded-[18px] shadow-[0_8px_24px_rgba(15,23,42,0.06)] p-3.5">
              ...
              <div class="text-[13.5px] font-semibold leading-snug mt-0.5 break-words">
                {{ formatShortVal(insight.daily_average) }}
              </div>
            </div>
            <div class="bg-white rounded-[18px] shadow-[0_8px_24px_rgba(15,23,42,0.06)] p-3.5">
    ...
    <Teleport to="body">
      ...
      >
        <div class="bg-white rounded-t-[24px] w-full max-w-md p-6 pb-10">
          ...
          <a
            :href="route('report.export-pdf', { period })"
            class="flex items-center gap-3 w-full p-4 bg-slate-50 rounded-[16px] mb-3"
          >
          ...
          <a
            :href="route('report.export-excel', { period })"
            class="flex items-center gap-3 w-full p-4 bg-slate-50 rounded-[16px] mb-3"
          >
            <span class="text-2xl">📊</span>
            <div class="text-left min-w-0">
          ...
          <button
            type="button"
            class="w-full py-3.5 rounded-[16px] text-[14px] font-medium text-slate-500 bg-slate-50"
...
const donutColors = [
  '#2F6BFF', '#60A5FA', '#818CF8', '#34D399',
  '#FBBF24', '#F87171', '#A78BFA', '#38BDF8',
]

const maxBarVal = computed(() => {
  const vals = props.barData.flatMap((d) => [d.income, d.expense])
  return Math.max(...vals, 1)
})

function barHeight(v) {
  return Math.max(3, (v / maxBarVal.value) * 100)
}
...
const healthColorText = computed(() => {
  const map = {
    sehat: 'text-emerald-500',
    cukup: 'text-amber-500',
    perlu_perhatian: 'text-rose-500',
  }
  return map[props.healthStatus] ?? 'text-slate-500'
})

function healthLabel(s) {
  const map = { sehat: 'Sehat', cukup: 'Cukup', perlu_perhatian: 'Perlu Perhatian' }
  return map[s] ?? s
}

function defaultTierEmoji(s) {
  const map = { sehat: '✅', cukup: '⚠️', perlu_perhatian: '🔴' }
  return map[s] ?? ''
}
...
function formatShortVal(n) {
  return formatShort(n)
}

const wastefulDayLabel = computed(() => {
  if (props.insight && props.insight.most_wasteful_day && props.insight.most_wasteful_day.date) {
    return props.insight.most_wasteful_day.date
  }
  return dash
})
</script>
