...
<template>
  <AppLayout>
    <div class="report-tw bg-slate-50 min-h-screen pb-8">
      ...
      <!-- ═══════════ HERO ═══════════ -->
      <div class="relative overflow-hidden bg-gradient-to-br from-[#2F6BFF] via-[#4F7FFF] to-[#1E4FD6] px-5 pt-6 pb-9 rounded-b-[28px]">
        <!-- Decorative pattern / glass blobs -->
        <div class="absolute -top-16 -right-10 w-56 h-56 rounded-full bg-white/10 blur-[2px]"></div>
        <div class="absolute top-20 right-16 w-24 h-24 rounded-full bg-white/10"></div>
        <div class="absolute top-4 left-1/3 w-20 h-20 rounded-full bg-white/5"></div>
        <svg class="absolute top-0 right-0 opacity-[0.07]" width="180" height="180" viewBox="0 0 180 180" fill="none">
          <circle cx="90" cy="90" r="1" fill="none" stroke="white" stroke-width="60" stroke-dasharray="2 8"/>
        </svg>

        <div class="relative z-10 flex justify-between items-start mb-5">
          <div>
            <h1 class="text-[26px] leading-tight font-bold text-white flex items-center gap-2">📊 Laporan</h1>
            <p class="text-white/75 text-[13px] mt-1.5 max-w-[210px] leading-snug">Pantau perkembangan keuanganmu setiap bulan.</p>
          </div>
          <div class="flex items-center gap-2">
            <button type="button" @click="showExportMenu = true"
              class="h-11 flex items-center gap-1.5 bg-white text-[#2F6BFF] font-semibold text-[13px] px-4 rounded-full shadow-[0_4px_14px_rgba(0,0,0,0.12)]">
              <Download :size="15" /> Export
            </button>
            <button type="button" class="relative h-11 w-11 rounded-full bg-white/15 backdrop-blur-sm flex items-center justify-center text-white shadow-[0_4px_14px_rgba(0,0,0,0.08)]">
              <Bell :size="18" />
        ...
        <div class="relative inline-block z-10">
          <button type="button" @click="showMonthMenu = !showMonthMenu"
            class="h-11 flex items-center gap-2 bg-white/15 backdrop-blur-sm text-white text-[13px] font-semibold px-4 rounded-full">
            <Calendar :size="15" /> {{ periodLabel }} <ChevronDown :size="15" :class="['transition-transform', showMonthMenu && 'rotate-180']" />
          </button>
      ...
      <div class="px-5 -mt-10 relative z-10">
        ...
        <div class="grid grid-cols-2 gap-4 mb-6">
          <div class="bg-white rounded-[24px] p-5 shadow-[0_4px_20px_rgba(15,23,42,0.06)]">
            <div class="w-11 h-11 rounded-2xl bg-emerald-50 flex items-center justify-center mb-3">
              <TrendingUp :size="20" class="text-emerald-500" />
            </div>
            <div class="text-[13px] text-slate-400 font-medium">Total Pemasukan</div>
            <div class="text-[22px] font-bold text-emerald-500 mt-1 leading-tight">{{ formatRupiah(totalIncome) }}</div>
            <div :class="['text-[12px] font-semibold mt-2 flex items-center gap-1', incomeChange >= 0 ? 'text-emerald-500' : 'text-rose-500']">
              <component :is="incomeChange >= 0 ? ArrowUp : ArrowDown" :size="11" /> {{ Math.abs(incomeChange) }}% vs bulan lalu
            </div>
          </div>
          <div class="bg-white rounded-[24px] p-5 shadow-[0_4px_20px_rgba(15,23,42,0.06)]">
            <div class="w-11 h-11 rounded-2xl bg-rose-50 flex items-center justify-center mb-3">
              <TrendingDown :size="20" class="text-rose-500" />
            </div>
            <div class="text-[13px] text-slate-400 font-medium">Total Pengeluaran</div>
            <div class="text-[22px] font-bold text-rose-500 mt-1 leading-tight">{{ formatRupiah(totalExpense) }}</div>
            <div :class="['text-[12px] font-semibold mt-2 flex items-center gap-1', expenseChange <= 0 ? 'text-emerald-500' : 'text-rose-500']">
              <component :is="expenseChange >= 0 ? ArrowUp : ArrowDown" :size="11" /> {{ Math.abs(expenseChange) }}% vs bulan lalu
            </div>
          </div>
        </div>

        <!-- ═══════════ SEGMENTED CONTROL ═══════════ -->
        <div class="relative bg-slate-100 rounded-full p-1 flex mb-6">
          ...
          <button type="button" v-for="opt in rangeOptions" :key="opt.value"
            ...
            :class="['relative z-10 flex-1 text-center text-[12px] font-bold py-2.5 rounded-full transition-colors', range === opt.value ? 'text-white' : 'text-slate-500']">
            {{ opt.label }}
          </button>
        </div>

        <!-- ═══════════ CHART CARD ═══════════ -->
        <div class="bg-white rounded-[24px] p-6 shadow-[0_4px_20px_rgba(15,23,42,0.06)] mb-6">
          <div class="flex justify-between items-center mb-6">
            <div class="text-[16px] font-bold text-slate-800">Pemasukan vs Pengeluaran</div>
            <button type="button" class="flex items-center gap-1 bg-slate-50 text-slate-500 text-[12px] font-semibold px-3 py-2 rounded-full">
              Bulanan <ChevronDown :size="13" />
            </button>
          </div>

          <div class="flex items-end gap-3 overflow-x-auto pb-2" style="height:170px;">
          ...
          <div class="flex gap-5 justify-center mt-5 pt-4 border-t border-slate-100">
          ...
          <div class="bg-white rounded-[24px] p-5 shadow-[0_4px_20px_rgba(15,23,42,0.06)]">
            <div class="flex justify-between items-start mb-4">
              <div class="flex items-center gap-2 text-[15px] font-bold text-slate-800">
                <Activity :size="17" class="text-[#2F6BFF]" /> Budget Health Score
              </div>
              <button type="button" class="w-8 h-8 rounded-full bg-slate-50 flex items-center justify-center">
                <ArrowRight :size="15" class="text-slate-400" />
              </button>
            </div>
            <div class="flex items-baseline gap-1 mb-1.5">
              <span class="text-[28px] font-bold text-[#2F6BFF] leading-none">{{ healthScore }}</span>
              <span class="text-[13px] text-slate-400">/100</span>
            </div>
            <div :class="['text-[12px] font-bold mb-4 flex items-center gap-1.5', healthColorText]">
              <AppIcon :slug="`health_tier_${healthStatus}`" class="w-4 h-4 inline-block">{{ defaultTierEmoji(healthStatus) }}</AppIcon>
            ...
            <div class="space-y-3">
              <div class="flex items-center gap-2 text-[12px]">
                ...
                <span class="w-9 text-right font-bold">{{ savingRatio }}%</span>
              </div>
              <div class="flex items-center gap-2 text-[12px]">
                ...
                <span class="w-9 text-right font-bold">{{ budgetDiscipline }}%</span>
              </div>
              <div class="flex items-center gap-2 text-[12px]">
                ...
                <span class="w-9 text-right font-bold">{{ runwayScore }}%</span>
              </div>
            </div>
          </div>
          ...
          <div class="bg-gradient-to-br from-slate-900 to-slate-800 text-white rounded-[24px] p-5 shadow-[0_4px_20px_rgba(15,23,42,0.15)] relative overflow-hidden">
            <div class="flex justify-between items-start mb-4 relative z-10">
              <div class="flex items-center gap-2 text-[15px] font-bold">
                <Shield :size="17" class="text-emerald-400" /> Dana Darurat
              </div>
              <button type="button" class="w-8 h-8 rounded-full bg-white/10 flex items-center justify-center">
                <ArrowRight :size="15" class="text-white/60" />
              </button>
            </div>
            <template v-if="runwayMonths !== null">
              <div class="flex items-baseline gap-1.5 mb-4 relative z-10">
                <span class="text-[28px] font-bold leading-none">{{ runwayMonths }}</span>
              ...
              <div class="text-[11px] text-white/45 relative z-10">Target ideal: 6 bulan pengeluaran</div>
            </template>
            <div v-else class="text-[12px] text-white/60 relative z-10">Belum cukup data.</div>
            <PiggyBank :size="80" class="absolute -bottom-4 -right-4 text-white/[0.08]" />
          </div>
        </div>

        <!-- ═══════════ DONUT CHART ═══════════ -->
        <div v-if="donutData.length > 0" class="bg-white rounded-[24px] p-6 shadow-[0_4px_20px_rgba(15,23,42,0.06)] mb-6">
          ...
          <div class="flex flex-col sm:flex-row items-center gap-8">
            <div class="relative w-44 h-44 flex-shrink-0">
              ...
              <div class="absolute inset-0 flex flex-col items-center justify-center">
                <div class="text-[11px] text-slate-400 font-medium">Total</div>
                <div class="text-[18px] font-bold">{{ formatShort(totalExpense) }}</div>
              </div>
            </div>
        ...
        <!-- ═══════════ BUDGET VS REALISASI ═══════════ -->
        <div v-if="budgets.length > 0" class="bg-white rounded-[24px] p-6 shadow-[0_4px_20px_rgba(15,23,42,0.06)] mb-6">
          <div class="text-[16px] font-bold text-slate-800 mb-5">Budget vs Realisasi</div>
        ...
        <div class="mb-2">
          ...
          <div class="grid grid-cols-2 gap-4">
            <div class="bg-white rounded-2xl p-4 shadow-[0_4px_20px_rgba(15,23,42,0.06)]">
              <div class="w-9 h-9 rounded-xl bg-emerald-50 flex items-center justify-center mb-2.5"><TrendingUp :size="16" class="text-emerald-500" /></div>
              <div class="text-[11px] text-slate-400 font-medium">Pemasukan tertinggi</div>
              <div class="text-[15px] font-bold mt-1">{{ insight.biggest_income ? formatShort(insight.biggest_income.amount) : '-' }}</div>
            </div>
            <div class="bg-white rounded-2xl p-4 shadow-[0_4px_20px_rgba(15,23,42,0.06)]">
              <div class="w-9 h-9 rounded-xl bg-rose-50 flex items-center justify-center mb-2.5"><TrendingDown :size="16" class="text-rose-500" /></div>
              <div class="text-[11px] text-slate-400 font-medium">Pengeluaran tertinggi</div>
              <div class="text-[15px] font-bold mt-1">{{ insight.biggest_expense ? formatShort(insight.biggest_expense.amount) : '-' }}</div>
            </div>
            <div class="bg-white rounded-2xl p-4 shadow-[0_4px_20px_rgba(15,23,42,0.06)]">
              <div class="w-9 h-9 rounded-xl bg-purple-50 flex items-center justify-center mb-2.5"><Users :size="16" class="text-purple-500" /></div>
              <div class="text-[11px] text-slate-400 font-medium">Rata-rata harian</div>
              <div class="text-[15px] font-bold mt-1">Rp {{ formatShort(insight.daily_average) }}</div>
            </div>
            <div class="bg-white rounded-2xl p-4 shadow-[0_4px_20px_rgba(15,23,42,0.06)]">
              <div class="w-9 h-9 rounded-xl bg-amber-50 flex items-center justify-center mb-2.5"><CalendarDays :size="16" class="text-amber-500" /></div>
              <div class="text-[11px] text-slate-400 font-medium">Hari paling boros</div>
              <div class="text-[15px] font-bold mt-1">{{ insight.most_wasteful_day?.date ?? '-' }}</div>
            </div>
          </div>
        </div>
...
const barHeight = (v) => Math.max(4, (v / maxBarVal.value) * 130)
...
<style scoped>
/* ── Reset kecil KHUSUS halaman ini — matiin border/appearance
   default browser tanpa nyentuh halaman lain (preflight tetap OFF global) ── */
.report-tw button,
.report-tw select,
.report-tw input,
.report-tw a {
