...
function patchFile(string $path, array $replacements): void
{
    ...
    if ($changed > 0) {
        ...
        echo "SKIP (pattern tidak ketemu): $path\n";
    }
}
...
patchFile($file, [
    ...
    '<div class="report-tw bg-slate-50 min-h-screen pb-8">' => '<div class="report-tw bg-slate-50 min-h-screen pb-8 overflow-x-hidden" style="max-width:100vw;">',
    ...
    'class="relative overflow-visible bg-gradient-to-br from-[#2F6BFF] via-[#4F7FFF] to-[#1E4FD6]
               pl-5 pr-[18px] pt-6 pb-14 rounded-b-[24px] shadow-[0_12px_40px_rgba(47,107,255,0.20)]"' => 'class="relative overflow-hidden bg-gradient-to-br from-[#2F6BFF] via-[#4F7FFF] to-[#1E4FD6]
               pl-5 pr-[18px] pt-5 pb-9 rounded-b-[24px] shadow-[0_12px_40px_rgba(47,107,255,0.20)]"',

    // ── Jarak dropdown ke Summary diperkecil ──
    'class="relative inline-block z-10 mt-3"' => 'class="relative inline-block z-10 mt-2.5"',

    // ── 2. SUMMARY: overlap lebih dalam (-28px -> -48px), gap diperlebar, padding & tinggi naik ──
    '<div class="px-5 relative z-20 grid grid-cols-2 gap-2.5" style="margin-top:-28px;">' => '<div class="px-5 relative z-20 grid grid-cols-2 gap-3" style="margin-top:-48px;">',
    ...
    '<div class="bg-white rounded-[18px] shadow-[0_8px_24px_rgba(15,23,42,0.10)] p-3 min-w-0">
          ...
          </div>' => '<div class="bg-white rounded-[20px] shadow-[0_8px_24px_rgba(15,23,42,0.10)] px-4 py-4 min-w-0">
    ...
    'class="relative bg-white rounded-full p-1 flex shadow-[0_4px_14px_rgba(15,23,42,0.06)]">' => 'class="relative bg-white rounded-full p-1.5 flex gap-1 shadow-[0_4px_14px_rgba(15,23,42,0.06)]">',
    ...
    'style="font-size:11px; min-height:38px;"
        ...
        <!-- ═══ CHART ═══ -->' => 'style="font-size:11.5px; min-height:48px;"
          >
            {{ opt.label }}
          </button>
        </div>
    ...
    'class="bg-white rounded-[24px] shadow-[0_8px_24px_rgba(15,23,42,0.06)] p-5">
          <div class="flex justify-between items-center mb-4 gap-2">
            <div class="font-semibold text-slate-800 text-left truncate min-w-0 flex-1" style="font-size:16px;">
              Pemasukan vs Pengeluaran' => 'class="bg-white rounded-[24px] shadow-[0_8px_24px_rgba(15,23,42,0.06)] px-5 pt-4 pb-5">
    ...
    'style="height:150px;">
              <div v-for="item in barData"' => 'style="height:180px;">
              <div v-for="item in barData"',
    'style="height:120px;">
                  <div
                    class="w-2.5 rounded-t-md bg-emerald-500' => 'style="height:150px;">
                  <div
                    class="w-2.5 rounded-t-md bg-emerald-500',

    'function barHeight(v) { return Math.max(3, (v / maxBarVal.value) * 120) }' => 'function barHeight(v) { return Math.max(3, (v / maxBarVal.value) * 150) }',
    ...
    'class="flex gap-4 justify-center pt-3 mt-1 border-t border-slate-100">
        ...
        <!-- ═══ HEALTH' => 'class="flex gap-4 justify-center pt-2 border-t border-slate-100">
            <div class="flex items-center gap-1.5 text-slate-500 font-normal" style="font-size:11px;">
              <span class="w-2 h-2 rounded-full bg-emerald-500"></span> Pemasukan
            </div>
            <div class="flex items-center gap-1.5 text-slate-500 font-normal" style="font-size:11px;">
    ...
    '<span class="font-semibold text-slate-800 text-left" style="font-size:16px;">Budget Health Score</span>
            ...
            <div
              :class="[\'font-medium mb-4 mt-1 flex items-center gap-1.5 text-left\', healthColorText]"' => '<span class="font-semibold text-slate-800 text-left" style="font-size:16px;">Budget Health Score</span>
            </div>
    ...
    'class="bg-gradient-to-br from-slate-900 to-slate-800 text-white rounded-[24px] shadow-[0_8px_24px_rgba(15,23,42,0.15)] p-5 relative overflow-hidden">
            ...
            <template v-if="runwayMonths !== null">
              <div class="text-left relative z-10 mb-3">' => 'class="bg-gradient-to-br from-slate-900 to-slate-800 text-white rounded-[24px] shadow-[0_8px_24px_rgba(15,23,42,0.15)] px-5 py-4 relative overflow-hidden">
    ...
    '<div v-for="(cat, i) in donutData" :key="cat.category" class="flex items-center gap-2" style="font-size:13px;">
              ...
              </div>' => '<div v-for="(cat, i) in donutData" :key="cat.category">
