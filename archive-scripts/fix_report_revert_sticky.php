...
patchFile($file, [
    ...
    'class="sticky top-0 z-30 overflow-visible bg-gradient-to-br from-[#2F6BFF] via-[#4F7FFF] to-[#1E4FD6]
        ...
        :class="isScrolled ? \'pt-4 pb-4\' : \'pt-6 pb-14\'"' => 'class="relative overflow-visible bg-gradient-to-br from-[#2F6BFF] via-[#4F7FFF] to-[#1E4FD6]
    ...
    'class="relative inline-block z-10 overflow-hidden transition-all duration-300 ease-out"
          :class="isScrolled ? \'max-h-0 opacity-0 mt-0\' : \'max-h-12 opacity-100 mt-3\'"' => 'class="relative inline-block z-10 mt-3"',

    // FIX 3: Format Rupiah pakai spasi lagi
    "function formatRupiah(n) {\n  return 'Rp' + Number(n || 0).toLocaleString('id-ID')\n}" => "function formatRupiah(n) {\n  return 'Rp ' + Number(n || 0).toLocaleString('id-ID')\n}",

    // FIX 4: Tambah "vs bulan lalu" balik ke indikator kenaikan/penurunan
    "{{ incomeChange >= 0 ? '▲' : '▼' }} {{ Math.abs(incomeChange) }}%\n          </div>" => "{{ incomeChange >= 0 ? '▲' : '▼' }} {{ Math.abs(incomeChange) }}% vs bulan lalu\n          </div>",

    "{{ expenseChange >= 0 ? '▲' : '▼' }} {{ Math.abs(expenseChange) }}%\n          </div>" => "{{ expenseChange >= 0 ? '▲' : '▼' }} {{ Math.abs(expenseChange) }}% vs bulan lalu\n          </div>",
